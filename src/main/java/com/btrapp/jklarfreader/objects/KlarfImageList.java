...
/**
 * Use this if you are supporting a (filePath,imgType,img,imgLabel) style record ex (
 * {"file01.jpg","JPG",1,"Patch"},{"file02.jpg","JPG",1,"Class-Topo"} )
 */
public class KlarfImageList {
  ...
  @Override
  public String toString() {
    if (images.isEmpty()) return "N";
    return "Images "
        + images.size()
        + " {"
        + images.stream().map(KlarfImageRow::toString).collect(Collectors.joining(", "))
        + "}";
  }
